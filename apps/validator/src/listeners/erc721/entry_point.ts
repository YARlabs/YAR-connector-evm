...
import { EvmListenerERC721 } from './EvmListenerERC721'
...
async function main() {
  ...
  const db = new DataBase({ dbName: 'ERC721', processName: `${CHAIN_NAME} EvmListener` })
  ...
  try {
    ...
    const listener = new EvmListenerERC721({
      name,
      bridgeAddress,
      numberOfBlocksToConfirm,
      providerUrls,
