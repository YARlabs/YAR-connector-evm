...
import { IssuedERC1155__factory, BridgeERC1155__factory } from 'typechains';
...
export const useGetSecondToken = () => {
    ...
    return useCallback(
        async (token: string, chainIdFrom: number, chainIdTo: number) => {
            ...
            let bridgeContract = BridgeERC1155__factory.connect(addressBridgeFrom, providerFrom);
            try {
                const isIssued = await bridgeContract.isIssuedTokenPublished(token);
                if(isIssued) {
                    let issuedTokenImplementationContract = IssuedERC1155__factory.connect(token, providerFrom);
                    ...
                    if(originalChainBytes32 === chainToBytes32) {
                        ...
                        bridgeContract = BridgeERC1155__factory.connect(addressBridgeTo, providerTo);
