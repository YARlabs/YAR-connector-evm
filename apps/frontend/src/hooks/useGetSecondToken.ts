...
import {IssuedERC20__factory, BridgeERC20__factory} from 'typechains';
...
export const useGetSecondToken = () => {
    ...
    return useCallback(
        async (token: string, chainIdFrom: number, chainIdTo: number) => {
            ...
            let bridgeContract = BridgeERC20__factory.connect(addressBridgeFrom, providerFrom);
            try {
                const isIssued = await bridgeContract.isIssuedTokenPublished(token);
                if(isIssued) {
                    let issuedTokenImplementationContract = IssuedERC20__factory.connect(token, providerFrom);
                    ...
                    if(originalChainBytes32 === chainToBytes32) {
                        ...
                        bridgeContract = BridgeERC20__factory.connect(addressBridgeTo, providerTo);
