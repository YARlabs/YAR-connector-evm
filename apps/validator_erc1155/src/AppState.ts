...
export class AppState {
  ...
  private static readonly _awaitingBatchTrasfersDB = 'AwatingBatchTransfers'
  public static async addAwatingBatchTrasfer(queueName: string, transfer: IBatchTransferModel) {
    const db = this._getDB(this._awaitingBatchTrasfersDB)
    const collection = db.collection(queueName)
    await collection.insertOne({
      transferId: transfer.transferId,
      inProgress: false,
  ...
  public static async getAwaitingBatchTrasfers(queueName: string, withoutInProgress: boolean = true) {
    const db = this._getDB(this._awaitingBatchTrasfersDB)
    const collection = db.collection(queueName)
    const awaitingTrasfers = await collection.find().toArray()
    const filteredTrasfers = withoutInProgress
  ...
  public static async completeAwaitingBatchTrasfer(queueName: string, transfer: IBatchTransferModel) {
    const db = this._getDB(this._awaitingBatchTrasfersDB)
    const collection = db.collection(queueName)
    ...
    {
      const db = this._getDB('CompletedBatchTransfers')
      const collection = db.collection(queueName)
