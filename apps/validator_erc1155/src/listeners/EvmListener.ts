...
export class EvmListener {
  ...
  constructor({
    ...
    this._bridgeERC1155 = BridgeERC1155__factory.connect(bridgeAddress, this._provider)
  ...
  public async init() {
    ...
    setInterval(async () => {
      ...
      for (const batchTransfer of batchTransfers) {
        this._addBatchTask(batchTransfer)
      }
  ...
  private async _addBatchTask(transfer: IBatchTransferModel) {
    let queueName = transfer.targetChain
    if (this._currentChain != this._proxyChain && transfer.targetChain != this._proxyChain)
    queueName = this._proxyChain
    await AppState.addAwatingBatchTrasfer(queueName, transfer)
  }
  ...
  private async _getBatchTransfers(
    startBlock: number,
    endBlock: number,
  ): Promise<Array<IBatchTransferModel>> {
    return (
      await this._bridgeERC1155.queryFilter(
