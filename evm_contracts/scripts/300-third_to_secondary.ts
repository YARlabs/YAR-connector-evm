...
const ethereumBridge = await UniversalBridgeContracts.factory(EthereumBridgeDeployment.address, ethereumValidator)
  ...
  const issuedTokenAddress = await thirdBridge.erc20Driver.getIssuedTokenAddressERC20(
  ...
  await thirdBridge.erc20Driver.connect(sender).tranferToOtherChainERC20(
